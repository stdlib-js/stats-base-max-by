...
tape( 'the function has an arity of 6', function test( t ) {
	t.strictEqual( maxBy.length, 6, 'has expected arity' );
	t.end();
});

tape( 'the function calculates the maximum value of a strided array via a callback function', function test( t ) {
	var x;
	var v;

	x = [ 1.0, -2.0, -4.0, 5.0, 0.0, 3.0 ];
	v = maxBy( x.length, x, 1, 0, accessor );
...
tape( 'the function calculates the maximum value of a strided array via a callback function (accessors)', function test( t ) {
	var x;
	var v;

	x = [ 1.0, -2.0, -4.0, 5.0, 0.0, 3.0 ];
	v = maxBy( x.length, toAccessorArray( x ), 1, 0, accessor );
	t.strictEqual( v, 10.0, 'returns expected value' );
...
tape( 'if provided an `N` parameter less than or equal to `0`, the function returns `NaN`', function test( t ) {
	...
	v = maxBy( 0, x, 1, 0, accessor );
	t.strictEqual( isnan( v ), true, 'returns expected value' );

	v = maxBy( -1, x, 1, 0, accessor );
	t.strictEqual( isnan( v ), true, 'returns expected value' );
...
tape( 'if provided an `N` parameter less than or equal to `0`, the function returns `NaN` (accessors)', function test( t ) {
	...
	v = maxBy( 0, toAccessorArray( x ), 1, 0, accessor );
	t.strictEqual( isnan( v ), true, 'returns expected value' );

	v = maxBy( -1, toAccessorArray( x ), 1, 0, accessor );
	t.strictEqual( isnan( v ), true, 'returns expected value' );
...
tape( 'the function supports a `stride` parameter (accessors)', function test( t ) {
	...
	v = maxBy( 4, toAccessorArray( x ), 2, 0, accessor );
...
tape( 'the function supports a negative `stride` parameter (accessors)', function test( t ) {
	...
	v = maxBy( 4, toAccessorArray( x ), -2, 6, accessor );
...
tape( 'if provided a `stride` parameter equal to `0`, the function returns the first accessed element', function test( t ) {
	...
	v = maxBy( x.length, x, 0, 0, accessor );
	t.strictEqual( v, 2.0, 'returns expected value' );

	x = new Array( 1 ); // eslint-disable-line stdlib/no-new-array

	v = maxBy( 1, x, 0, 0, accessor );
	t.strictEqual( isnan( v ), true, 'returns expected value' );
...
tape( 'if provided a `stride` parameter equal to `0`, the function returns the first accessed element (accessors)', function test( t ) {
	...
	v = maxBy( x.length, toAccessorArray( x ), 0, 0, accessor );
	t.strictEqual( v, 2.0, 'returns expected value' );

	x = new Array( 1 ); // eslint-disable-line stdlib/no-new-array

	v = maxBy( 1, toAccessorArray( x ), 0, 0, accessor );
	t.strictEqual( isnan( v ), true, 'returns expected value' );
...
tape( 'the function supports providing a callback execution context', function test( t ) {
	...
	maxBy( x.length, x, 1, 0, accessor, ctx );
